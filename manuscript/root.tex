...
Deep reinforcement learning and multi-joint dynamic simulation methods may help overcome this control bottleneck.  However, these systems tend to operate over large continuous action spaces and exhibit sparse rewards, thus making control policy exploration non-trivial and highly variable.    

Using the observations that an underactuated system has coupled states and that a deep function approximator may generalize across compressed (and potentially nonintuitive) states, we consider a reduced order approach based around a generative autoencoder so as to automatically find a better representation to focus a control policy exploration process. 
