...
We use the intuition that an underactuated system has relations and constraints correlations between dynamic states %(otherwise it would be uncontrollable for said states)
...
%Reinforcement Learning algorithm we use is
...
\begin{figure}[t]
    ...
    \caption{
        ...
        %All plots are filtered with a 100{\tiny E}\textsuperscript{3} step moving average filter.
    }
    \label{fig:ae-lowvar}
\end{figure}
...
% The more information one has the more that can be exploited, but also the more that has to be explored. 
%The more information one has, the more there is to exploit, but the more there is to explore.
...
We consider (in Fig. \ref{fig:rl-perf}) the benchmark performance of the various tasks in the Jitterbug problem task suite using both DDPG \cite{DDPG} (an off-policy method) and PPO \cite{PPO} (an on-policy method).  We find that DDPG is able to find rewards more stably, which we assume is because of its replay buffer. %Exectution time was capped to 8 hours so as to represent possible execution on a `field' deployable system.  The cases were stopped when the problem is solved or when it ran out of execution time.
...
%     \vspace*{-6pt}
%     \centering
%     \includegraphics[width=\linewidth]{fig-rl-perf_CROP2}
...
\begin{figure*}[h]
	...
	\centering
	\includegraphics[height=0.9\textheight]{fig-rl-perf_super}
	...
	\caption{
		...
		We show median (solid line) and the 10\textsuperscript{th} and 90\textsuperscript{th} quartiles (shaded area) of per-episode episode return across 10 random seeds in each figure.
		%A task is considered `solved' if the trained agent consistently scores $\gtrapprox 900$ return per episode (red line).  
		All plots are filtered with a 20{\tiny E}\textsuperscript{3} step moving average filter.  %For the cases where the results stop early (noted with a {\LARGE$\ast$}), this is because the simulations were limited to have the same compute time for comparison. 
	}    
	\label{fig:rl-perf}
\end{figure*}
