...
class Autoencoder():
    ...
    def __init__(self,
        ...
        self.X_noise = tf.placeholder(tf.float32, shape=[None, num_inputs])
        ...
        self.hid_layer1 = actf(tf.matmul(self.X_noise, w1) + b1)
    ...
    def train_autoencoder(self,
        ...
        for epoch in range(num_epoch):
            ...
            for iteration in range(num_batches):
                ...
                X_batch_noise = X_batch + noise_factor * np.random.normal(loc=0.0, scale=1.0, size=N_col)
                self.sess.run(self.train, feed_dict={self.X: X_batch, self.X_noise: X_batch_noise})

            train_loss = self.loss.eval(session=self.sess,feed_dict={self.X: X_batch, self.X_noise: X_batch_noise})
    ...
    def evaluate_autoencoder(self, testing_data):
        ...
        predictions = self.sess.run(self.output_layer, feed_dict={self.X:testing_data, self.X_noise:testing_data})
    ...
    def encode(self, obs):
        ...
        reduced_obs = self.sess.run(self.hid_layer1, feed_dict={self.X: obs, self.X_noise: obs})
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        autoencoder.train_autoencoder(training_data=training_data,
                                      num_epoch=200,
                                      batch_size=8192,
                                      save_path="./autoencoder_model21.ckpt",
                                      noise_factor=0.1
                                      )
